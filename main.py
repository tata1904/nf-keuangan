...
if selected == 'Profile':
        ...
        if tambah:
            if nama_akun.strip() == "" or (debit == 0 and kredit == 0):
                ...
                row = {
                    "No": nomor,
                    "Nama Akun": nama_akun,
                    "Debit (Rp)": float(debit),
                    "Kredit (Rp)": float(kredit)
                }
    ...
    if st.session_state.df_neraca_saldo_setelah_penutup.empty:
        ...
        edited_df["Debit (Rp)"] = pd.to_numeric(edited_df["Debit (Rp)"], errors="coerce").fillna(0).astype(float)
        edited_df["Kredit (Rp)"] = pd.to_numeric(edited_df["Kredit (Rp)"], errors="coerce").fillna(0).astype(float)
